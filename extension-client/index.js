let baseURL = "http://localhost:3000/result/";  //Base url for get request to server
...
function setCookie(str) {
  chrome.cookies.set({
    ...
    if (chrome.runtime.lastError) console.error("Error setting cookie:", chrome.runtime.lastError);
...
function removeDarkClassFromAllElements() {
  const elements = document.getElementsByTagName('*');
  for (let i = 0; i < elements.length; i++) elements[i].classList.remove('dark');
}
...
function process() {
  chrome.tabs.query({ active: true, lastFocusedWindow: true }, tabs => {
    ...
    {
      document.getElementById("error_msg").innerHTML = "Not a YouTube page...!!";
      hide("loader");
      show("error");
    }
  });
...
function calculateTime(url) {
  ...
  {
    document.getElementById("error_msg").innerHTML = "Not a YouTube playlist page...<br>Please visit and try again !!"
    hide("loader");
    show("error");
    return;
  }
...
async function getTime(id) {
  ...
  {
    console.log(error);
    document.getElementById("error_msg").innerHTML = "Uhh... Something went wrong <br>Please try again later !!";
    hide("loader");
    show("error");
    return;
  }
...
async function init() {
  ...
  for (let i = 0; i < element.length; i++) element[i].addEventListener("click", toggleMode);

  element = document.getElementsByClassName("reload");
  for (let i = 0; i < element.length; i++) element[i].addEventListener("click", refresh);
